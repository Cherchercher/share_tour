...
import EventModel from '../components/UI/EventModel';
...
import './Venue.css';
import { getInterestsByTourId } from '../actions/interest.actions';
import { getEventsByTourId } from '../actions/event.actions';
...
const VenuePage = (props) => {
    ...
    const [showAddEventModal, setShowAddEventModal] = useState(true);
    const [bookingModalShow, setBookingModalShow] = useState(false);
    const [interestModalShow, setInterestModalShow] = useState(false);
    const [interestsCalendarShow, setInterestsCalendarShow] = useState(false);
    const [bookingsCalendarShow, setBookingsCalendarShow] = useState(true);
    const [events, setEvents] = useState([]);
    const [isEventListSet, setIsEventListSet] = useState(false);
    const oneVenueInfo = useSelector(state => state.oneVenueInfo);
    const eventsInfo = useSelector(state => state.eventsInfo);
    const interestsInfo = useSelector(state => state.interestsInfo);
    ...
    useEffect(() => {
        console.log("getting events for Id", _id)
        dispatch(getEventsByTourId(_id));
    }, []);
    ...
    if (interestsInfo.loading) {
        return (
            <Layout>
                <div className='text-center' style={{ marginTop: '60px' }}>
                    <h1>Getting interests info 🎉</h1>
                    <Spinner animation="border" variant="success" />
                </div>
            </Layout>
        );
    ...
    // if (eventsInfo.loading) {
    //     return (
    //         <Layout>
    //             <div className='text-center' style={{ marginTop: '60px' }}>
    //                 <h1>Getting Events info 🎉</h1>
    //                 <Spinner animation="border" variant="success" />
    //             </div>
    //         </Layout>
    //     );
    ...
    const showInterestsCalendar = () => {
        if (!auth.authenticate) {
            history.push("/signin");
        } else {
            if (interestsCalendarShow === false && isEventListSet === false) {
                setIsEventListSet(true);
                dispatch(getInterestsByTourId(_id));
            }
            setInterestsCalendarShow(!interestsCalendarShow);
       }
    }
                ...
                <section className="mb-5">
                    <div className="row">
                        ...
                        <div className="col-md-6">
...

                            {
                                auth.user.role === "client" ?
                                    <>
                                        ...
                                        <p>Don't see a date/price that works for you?</p>
                                        <Button variant="primary" onClick={() => showInterest()}>Express Interest</Button>

                                        <p>{interestsInfo.numberOfInterests} people interested</p>
                                        <Button variant="info" onClick={() => showInterestsCalendar()}>View/Express Interests Calendar</Button>
                                    </>
                                    :
                                    <>
                                        ...
                                        <Button variant="info" onClick={() => showInterestsCalendar()}>View Interests Calendar</Button>
                                    </>
                            }

                              <RevoCalendar
                                events={eventsInfo.allEvents}
                                style={
                                    {borderRadius:"5px",
                                    border:"5px solid #4F6995",
                                    display: bookingsCalendarShow ? "flex": "none",
                                }}
                                highlightToday={true}
                                lang="en"
                                primaryColor="#4F6995"
                                ...
                                allowDeleteEvent={auth.user.role === "client" ? false : true}
                                allowAddEvent={auth.user.role === "client" ? false : true}
                            ...
                            <RevoCalendar
                                events={interestsInfo.allInterests}
                                style={
                                    {borderRadius:"5px",
                                    border:"5px solid #4F6995",
                                    display: interestsCalendarShow ? "flex": "none",
                                }}
                                highlightToday={true}
                                lang="en"
                                primaryColor="#4F6995"
